 ...
 function DriverloginAuthentication(req,res){
  ...
  if(result.length){
   ...
   bcrypt.compare(data.password,hash).then(function(result1){
    ...
    DriverModal.UpdateDriversAvailabilty({available:"YES",id:result[0].id},(err2,result2)=>{
      if(err){
        console.log(err);
        res.status(400).send({message:"ERROR IN UPDATING THE DRIVERS AVAILABILITY STATUS AFTER LOGIN"})
      }   
     }); 
